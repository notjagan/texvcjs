...
function parseNewCommand(definition, command, cmds) {
    ...
    if (sections.length === 0) {
        ...
        var cmd = definition.replaceAll(sections[0][0], "").match(/\\[a-zA-Z]+/)[0];
...
function parseSty(inp, cmds) {
    ...
    while (lineno < lines.length) {
        ...
        while (balance !== 0 || (lineno + 1 < lines.length && lines[lineno + 1].charAt(0) !== "\\")) {
            lineno++;
...
try{ //try catch to make sure program still runs without csv file
    ...
    parseSty(fs.readFileSync(path.join(__dirname, "texvc.sty"), 'utf8'), module.exports.other_literals3);
