...
function parseSpecFun(text, pegjs, ast, astutil, render) {
    var sections = findBracketSections(text);
    var command = sections[0][0].replace("\\", "\\\\");
    var i = 0;
    ...
    var replacement = sections[2 + i][0];
    var meaning = sections[3 + i][0];
    if (sections[4 + i][0].indexOf("meaning=") != -1) {
        console.log(command);
        var argumentCount = parseInt(sections[5 + i][0]);
    } else {
        var argumentCount = parseInt(sections[4 + i][0]);
    }
    var pegjscode;
    var astcode;
    var astutilcode1;
    ...
    var rendercode;
    var cmd;
    if (sections.length > 5 + i) {
        for (var r = 5 + i; r < sections.length; r++) {
            cmd = command + (r - 3 - i)
            astcode = cmd + ": { args: [ 'string', ";
            astutilcode1 = cmd + ": function(target, f, ";
            astutilcode2 = "    return match(target, f) || ";
            rendercode1 = cmd + ": function(";
            rendercode2 = "    return \"" + replacement + "\" + ";
            pegjscode = "  / " + "name:\"\\" + command + "\" ";
            var temp = "";
            for (var par = 0; par < parameterCount; par++) {
                pegjscode += "l" + (par + 1) + ":lit ";
                if (par == parameterCount - 1 && argumentCount == 0) {
                    temp += "l" + (par + 1);
                    astcode += "'self'] },";
                    astutilcode2 += "l" + (par + 1) + ".contains_func(target);\n";
                    astutilcode1 += "l" + (par + 1) + ") {\n\t";
                    rendercode1 += "l" + (par + 1) + ") {\n\t"
                } else {
                    temp += "l" + (par + 1) + ", ";
                    astcode += "'self', ";
                ...
                rendercode2 = rendercode2.replace("#" + (par + 1), "\" + l" + (par + 1) + " + \"");
            }
            rendercode2 += "\"" + sections[r][0] + "\"";
            for (var at = 0; at < r - 4 - i; at++) {
                pegjscode += "atsymbol ";
            }
            for (var arg = 0; arg < argumentCount; arg++) {
                pegjscode += "k" + (arg + 1) + ":lit ";
                if (arg == argumentCount - 1) {
                    temp += "k" + (arg + 1);
                    astcode += "'self'] },";
                    astutilcode2 += "k" + (arg + 1) + ".contains_func(target);\n";
                    astutilcode1 += "k" + (arg + 1) + ") {\n\t";
                    rendercode1 += "k" + (arg + 1) + ") {\n\t"
                } else {
                    temp += "k" + (arg + 1) + ", ";
                    astcode += "'self', ";
                ...
                rendercode2 = rendercode2.replace("#" + (arg + 1), "\" + k" + (arg + 1) + " + \"");
            }
        }
    }
    else {
    ...
    astutilcode = astutilcode1 + astutilcode2 + "    },";
    rendercode = (rendercode1 + rendercode2 + ";\n    },").replaceAll("\\", "\\\\", false);
    pegjscode += " { return ast.Tex." + cmd + "(name, " + temp + "); }";
...
function parseSty(sty, pegjs, ast, astutil, render) {
    ...
    while (first !== -1) {
        ...
        if (prev === "\\defSpecFun") {
            parseSpecFun(text, pegjs, ast, astutil, render);
        }
        else {
...
try{ //try catch to make sure program still runs without csv file'
    ...
    var astutil = [];
    var render = [];
    module.exports.other_literals3 = parseSty("DRMFfcns.sty", pegjs, ast, astutil, render);
    ...
    var fs = require('fs');
    var path = require('path');
    fs.readFile(path.join(__dirname, "parser.pegjs"), 'utf8', function (err, data) {
        for (var i in [0, 1]) {
          var start = ["lit\n  =", "litstuff ="][i];
          if (err) {
          ...
          index = data.indexOf("\n", index) + 1;
          var result = data.substring(0, index) + pegjs.join("\n") + "\n" + data.substring(index);
          result = result.replace("require('./ast');", "require('./ast-copy');");
          fs.writeFile("parser-copy.pegjs", result, 'utf8', function (err) {
    ...
    console.log("hi");
    fs.readFile(path.join(__dirname, "ast.js"), 'utf8', function (err, data) {
        if (err) {
            return console.log(err);
        }
        ...
        fs.writeFile("ast-copy.js", result, 'utf8', function (err) {
            if (err) return console.log(err);
        });
    });
    fs.readFile(path.join(__dirname, "astutil.js"), 'utf8', function (err, data) {
        if (err) {
            return console.log(err);
        }
        ...
        result = result.replace('require("./ast");', 'require("./ast-copy");');
        fs.writeFile("astutil-copy.js", result, 'utf8', function (err) {
            if (err) return console.log(err);
        });
    });
    fs.readFile(path.join(__dirname, "render.js"), 'utf8', function (err, data) {
        if (err) {
            return console.log(err);
        }
        ...
        result = result.replace("require('./ast');", "require('./ast-copy');");
        fs.writeFile("render-copy.js", result, 'utf8', function (err) {
            if (err) return console.log(err);
        });
    });
    fs.readFile(path.join(__dirname, "build-parser.js"), 'utf8', function (err, data) {
        if (err) {
            return console.log(err);
        }
        var result = data.replace("'/parser.pegjs'", "'/parser-copy.pegjs'");
        result = result.replace("'/parser.js'", "'/parser-copy.js'");
        fs.writeFile("build-parser-copy.js", result, 'utf8', function (err) {
            if (err) return console.log(err);
        });
    });
    fs.readFile(path.join(__dirname, "parser.js"), 'utf8', function (err, data) {
        if (err) {
            return console.log(err);
        }
        var result = data.replace("'/ast.js'", "'/ast-copy.pegjs'");
        fs.writeFile("parser-copy.js", result, 'utf8', function (err) {
            if (err) return console.log(err);
        });
    });
