...
function parseSpecFun(text, pegjs, 0ast, astutil, render) {
    ...
    var replacement = sections[1 + i][0];
    var meaning = sections[2 + i][0];
    if (sections[3+i][0].indexOf("meaning=") != -1) {
        var argumentCount = parseInt(sections[3 + i + 1][0]);
    } else {
        var argumentCount = parseInt(sections[3 + i][0]);
    }
    var pegjscode;
    var astcode;
    var astutilcode1;
    ...
    var rendercode;
    var cmd;
    for (var r = 4 + i; r < sections.length; r++) {
        cmd = command + (r - 3 - i)
...
try{ //try catch to make sure program still runs without csv file
    ...
    fs.readFile("ast.js", 'utf8', function (err, data) {
        if (err) {
            return console.log(err);
        }
    ...
    fs.readFile("astutil.js", 'utf8', function (err, data) {
        if (err) {
            return console.log(err);
        }
    ...
    fs.readFile("render.js", 'utf8', function (err, data) {
        if (err) {
            return console.log(err);
        }
